...
from lavis.processors.alpro_processors import AlproVideoBaseProcessor, ToUint8, ToTHWC
...
@registry.register_processor("blip2_pseudo_video_train")
class Blip2PseudoVideoTrainProcessor(BlipImageBaseProcessor):
    def __init__(
        self, image_size=364, n_frms=8, same_aug=True, mean=None, std=None, min_scale=0.5, max_scale=1.0
    ):
        super().__init__(mean=mean, std=std)
        self.n_frms = n_frms
        self.same_aug = same_aug
        self.transform = transforms.Compose(
            [
                transforms.RandomResizedCrop(
                    image_size,
    ...
    def __call__(self, item):
        if self.same_aug:
            return self.transform(item).unsqueeze(1).repeat(1, self.n_frms,1,1)
        else:
            return torch.cat([self.transform(item).unsqueeze(1) for x in range(self.n_frms)], 1)
    @classmethod
    def from_config(cls, cfg=None):
        if cfg is None:
            cfg = OmegaConf.create()
        image_size = cfg.get("image_size", 364)
        n_frms = cfg.get("n_frms", None)
        mean = cfg.get("mean", None)
        std = cfg.get("std", None)
        same_aug = cfg.get("same_aug", True)
        min_scale = cfg.get("min_scale", 0.5)
        max_scale = cfg.get("max_scale", 1.0)
        return cls(
            image_size=image_size,
...
@registry.register_processor("blip_video_train")
class BlipVideoTrainProcessor(AlproVideoBaseProcessor):
    def __init__(
        self,
        image_size=384,
        mean=None,
...
class BlipVideoEvalProcessor(AlproVideoBaseProcessor):
    def __init__(self, image_size=256, mean=None, std=None, n_frms=100, flip=False):
        super().__init__(mean=mean, std=std, n_frms=n_frms)
        ...
        self.transform = transforms.Compose(
            [
                ...
                transforms.RandomHorizontalFlip(int(flip)),
                ToUint8(),  # C, T, H, W
                ToTHWC(),  # T, H, W, C
    ...
    def from_config(cls, cfg=None):
        ...
        return cls(image_size=image_size, mean=mean, std=std, n_frms=n_frms, flip=flip)
