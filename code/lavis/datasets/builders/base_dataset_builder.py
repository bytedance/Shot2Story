...
class BaseDatasetBuilder:
    ...
    # customed for audio-visual video-text pretrain
    def build_custom_splits(self):
        """
        ...
        datasets = dict()
        for split_ori in ann_info.keys():
            if 'train' in split_ori:
                split = 'train'
            elif 'val' in split_ori:
                split = 'val'
            elif 'test' in split_ori:
                split = 'test'
            ...
            # annotation path
            ann_paths = ann_info.get(split_ori).storage
            if isinstance(ann_paths, str):
                ann_paths = [ann_paths]

            abs_ann_paths = []
            for ann_path in ann_paths:
                if not ann_path.startswith('hdfs') and not os.path.isabs(ann_path):
            ...
            datasets[split_ori] = dataset_cls(
                ...
                fix_total=fix_total,
                flexible_sampling=flexible_sampling,
            )
