...
@registry.register_model("blip_caption_video")
class BlipCaptionVideo(BlipBase):
    """
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        "base_coco": "configs/models/blip_caption_video_base_coco.yaml",
        "large_coco": "configs/models/blip_caption_video_large_coco.yaml",
    }
    ...
    def forward_encoder(self, samples):
        video = samples["video"]
        bs = video.size(0)
        n_frame = video.size(2)
        h = video.size(3)
        w = video.size(4)
        # N C T HW
        video = torch.permute(video, (0, 2, 1, 3, 4)).reshape((n_frame*bs, -1 , h, w))
        image_embeds = self.visual_encoder.forward_features(video)
        print('image embeds', image_embeds.size())
        image_embeds = torch.reshape(image_embeds, (bs, n_frame) + image_embeds.size()[1:])
