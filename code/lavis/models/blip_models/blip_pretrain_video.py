...
@registry.register_model("blip_pretrain_video")
class BlipPretrainVideo(BlipBase, SharedQueueMixin, MomentumDistilationMixin):
    """
    BLIP pretrain model.
    ...
    def forward(self, samples):
        ...
        h = video.size(2)
        w = video.size(3)
        video = torch.reshape(video, (self.n_frames*batch_size, h, w, -1))
        image_embeds = self.visual_encoder.forward_features(video)
        image_embeds = torch.reshape(image_embeds, (batch_size, self.n_frames, h, w, -1))
        video_embeds = torch.mean(image_embeds, 1)
        image_atts = torch.ones(video_embeds.size()[:-1], dtype=torch.long).to(
            video.device
        )
        ...
        with torch.no_grad():
            ...
            image_embeds_m = self.visual_encoder_m(video)
            image_embeds_m = torch.reshape(image_embeds_m, (batch_size, self.n_frames, video.size(2), video.size(3), -1))
            video_embeds_m = torch.mean(image_embeds_m, 1)

            video_feat_m = F.normalize(
                self.vision_proj_m(video_embeds_m[:, 0, :]), dim=-1
            )
            video_feat_all = torch.cat(
